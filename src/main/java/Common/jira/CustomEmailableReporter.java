    ...
    private void resultSummary(ISuite suite, IResultMap tests, String testname, String style, String details) {
        if (tests.getAllResults().size() > 0) {
            ...
            for (ITestNGMethod method : getMethodSet(tests, suite)) {
                ...
                String testInstanceName = resultSet.toArray(new ITestResult[resultSet.size()])[0].getTestName();
                buff.append("<td><a href=\"#m").append(m_methodIndex).append("\">").append(qualifiedName(method))
                        .append(" ")
                        .append(description != null && description.length() > 0 ? "(\"" + description + "\")" : "")
                        .append("</a>").append(null == testInstanceName ? "" : "<br>(" + testInstanceName + ")")
                        .append("</td>").append("<td class=\"numi\">").append(resultSet.size()).append("</td>")
